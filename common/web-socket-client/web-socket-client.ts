...
export class WebSocketClient {
    ...
    // E-H2 FIX: Track consecutive reconnect failures for backoff
    private _reconnectFailures = 0;
    private static readonly MAX_RECONNECT_BACKOFF_MS = 60000; // 1 minute max
    ...
    async bind(url: string) {
        ...
        this._pingInterval = setInterval(() => {
            if (
                ...
                this._pingPromises = [];
                // E-H2: Exponential backoff on consecutive failures
                const delay = Math.min(
                    1000 * Math.pow(2, this._reconnectFailures),
                    WebSocketClient.MAX_RECONNECT_BACKOFF_MS
                );
                this._reconnectFailures++;
                console.log(`[WS] Reconnect attempt ${this._reconnectFailures}, backoff ${delay}ms`);
                setTimeout(() => {
                    this._connect(url).catch(() => {
                        console.log('[WS] Reconnect failed, will retry on next ping interval');
                    });
                }, delay);
            } else {
    ...
    private async _connect(url: string) {
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.onopen = () => {
                this._reconnectFailures = 0; // E-H2: Reset backoff on success
