...
const useSubtitlePlayerStyles = makeStyles<Theme, StylesProps, string>((theme) => ({
    container: {
        ...
        overflowX: 'hidden',
        overflowY: 'auto',
        scrollBehavior: 'smooth', // Buttery smooth scrolling
...
const useSubtitleRowStyles = makeStyles<Theme>((theme) => ({
    subtitleRow: {
        ...
        minWidth: 250,
        // Smooth transition for highlight changes
        '& > td': {
            transition: 'background-color 0.4s ease-out, border-color 0.3s ease-out',
        },

        // The current subtitle is marked by imperatively toggling `Mui-selected`.
        // MUI applies a default selected background via `.Mui-selected` on TableRow.
        // We override it here to ensure **NO filled background**, only an outline.
        '&.Mui-selected': {
        ...
        // Outline: apply borders to the row cells.
        '&.Mui-selected > td': {
            backgroundColor: 'rgba(26, 178, 216, 0.15) !important',
            borderTop: 'none !important',
            borderBottom: 'none !important',
            borderLeft: 'none !important',
            borderRight: 'none !important',
            // Ensure no box-shadow or outline interferes
            boxShadow: 'none !important',
            outline: 'none !important',
        },
    },

    // NOTE: Keep these classes for selection-range UI, but do not fill background.
    selectedSubtitleRow: {
        minWidth: 250,
        '& td': {
            borderTop: `2px solid rgba(6, 182, 212, 0.45)`,
            borderBottom: `2px solid rgba(6, 182, 212, 0.45)`,
            borderLeft: 'none',
            borderRight: 'none',
        },
...
const SubtitleRow = React.memo(function SubtitleRow({
    ...
    if (subtitle.track && subtitle.track > 0) {
        className = classes.secondarySubtitle;
    } else {
        className = compressed ? classes.compressedSubtitle : classes.subtitle;
    }
...
interface SubtitlePlayerProps {
    ...
    /**
     * Optional hook: when a colored Metheus token inside the subtitle list is clicked.
     * Used by the Side Panel to open the PAGE popup anchored next to the browser side panel.
     */
...
export default function SubtitlePlayer({
        ...
        const update = () => {
            ...
            if (!keysAreEqual(currentSubtitleIndexes, highlightedSubtitleIndexesRef.current)) {
                ...
                if (smallestIndex !== undefined) {
                    ...
                    const allowScroll = !hiddenRef.current && Date.now() - lastScrollTimestampRef.current > 3000;
    ...
    useEffect(() => {
        ...
        table?.addEventListener('scroll', handleScroll, { passive: true }); // Catch scrollbar too
    ...
    const handlePointerDownCapture = useCallback(
        (e: React.PointerEvent) => {
            // Intercept clicks on Metheus tokens rendered inside dangerouslySetInnerHTML.
            // This is more reliable than a document-level listener because the subtitle list
            // may live inside complex React/MUI trees.
            ...
            // Prevent the row click handler from seeking when clicking a word.
            e.preventDefault();
            e.stopPropagation();
