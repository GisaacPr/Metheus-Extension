...
export interface MetheusSettings {
    /** Base URL of the Metheus platform (e.g., https://metheus.app) */
    readonly metheusUrl: string;

    /** API Key for authentication (starts with ln_) */
    readonly metheusApiKey: string;

    /** User ID from Firebase/Metheus */
    readonly metheusUserId: string;

    /** @deprecated Use metheusApiKey instead */
    readonly metheusToken: string;

    /** Whether integration is enabled */
    readonly metheusEnabled: boolean;

    /** Sync known words with platform */
    readonly metheusSyncKnownWords: boolean;

    /** Auto-export mined cards to platform */
    readonly metheusAutoExport: boolean;

    /** Target deck ID on the platform (optional) */
    readonly metheusTargetDeckId: string;
    ...
    /** Whether to enrich dictionary results from online sources (Free Dictionary, Wiktionary, etc.) */
    readonly metheusOnlineDictionary: boolean;
}
...
export interface DictionaryTrack {
    ...
    readonly dictionaryTokenMatchStrategy: string;
    readonly dictionaryTokenMatchStrategyPriority: string;
    readonly dictionaryYomitanUrl: string;
    readonly dictionaryYomitanScanLength: number;
    readonly dictionaryTokenReadingAnnotation: string;
    readonly dictionaryAnkiWordFields: string[];
    readonly dictionaryAnkiSentenceFields: string[];
    readonly dictionaryAnkiSentenceTokenMatchStrategy: string;
    readonly dictionaryAnkiMatureCutoff: number;
    readonly dictionaryAnkiTreatSuspended: string | number;
    readonly tokenStyling: string;
    readonly tokenStylingThickness: number;
    readonly colorizeFullyKnownTokens: boolean;
    readonly tokenStatusColors: string[];
}

export const dictionaryTrackHoverOnly = (track?: DictionaryTrack): boolean => {
    return track?.dictionaryColorizeOnHoverOnly ?? false;
};
