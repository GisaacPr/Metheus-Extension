...
type (
	forwarder struct {
		...
		ResponseChannels  map[string]chan clientResponse // E-C3 FIX: per-messageId channels
		Mutex             *sync.Mutex
		...
		for {
			...
			if err != nil {
				...
				if msg == "PING" {
					...
					if err == nil {
						// E-C3 FIX: Route response to the correct per-messageId channel
						forwarder.Mutex.Lock()
						if ch, ok := forwarder.ResponseChannels[response.MessageId]; ok {
							ch <- response
						}
...
func (forwarder forwarder) publishMessageAndAwaitResponse(command clientCommand, c chan clientResponse) {
	// E-C3 FIX: Register a per-messageId channel before publishing
	forwarder.Mutex.Lock()
	respChan := make(chan clientResponse, 1)
	forwarder.ResponseChannels[command.MessageId] = respChan
	forwarder.Mutex.Unlock()

	// Cleanup on exit
	defer func() {
		forwarder.Mutex.Lock()
		delete(forwarder.ResponseChannels, command.MessageId)
		forwarder.Mutex.Unlock()
	}()
...
func (forwarder forwarder) forwardToAnkiConnect(buf *bytes.Buffer, c echo.Context, method string) error {
	...
	// E-L6 FIX: Check error before using ankiConnectRequest
	if err != nil {
		return err
	}
	...
	// E-L7 FIX: Ensure response body is closed
	defer ankiConnectResponse.Body.Close()
	...
	// E-L7 FIX: Guard against nil Content-Type header
	contentType := "application/octet-stream"
	if ct := ankiConnectResponse.Header["Content-Type"]; len(ct) > 0 {
		contentType = ct[0]
	}
	c.Blob(ankiConnectResponse.StatusCode, contentType, ankiConnectResponseBuf.Bytes())
	return nil
}
...
func (forwarder forwarder) handleGetRequest(c echo.Context) error {
	...
	if err != nil {
		c.Logger().Error(err)
		// E-L6 FIX: Return error to stop execution
	...
	return c.Blob(ankiConnectResponse.StatusCode, contentType, ankiConnectResponseBuf.Bytes())
}
...
func main() {
	...
	forwarder := forwarder{
		...
		ResponseChannels: make(map[string]chan clientResponse),
