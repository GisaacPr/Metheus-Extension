...
import { base64ToBlob, bufferToBase64 } from '@metheus/common/base64';
import { normalizeLangCode, detectLanguage } from '../services/language-utils';
...
export default class VideoDataSyncController {
    ...
    private async _buildModel(additionalFields: Partial<VideoDataUiModel>) {
        ...
        const hasMetheusAuth = !!(
            (await this._context.settings.getSingle('metheusApiKey')) ||
            (await this._context.settings.getSingle('metheusToken'))
        );
        const targetLanguage = hasMetheusAuth
            ? await this._context.settings.getSingle('metheusTargetLanguage')
            : undefined;
        const platformLanguage = await this._context.settings.getSingle('language');
        const subs = await this._matchLastSyncedWithAvailableTracks(targetLanguage, platformLanguage);
    ...
    private async _matchLastSyncedWithAvailableTracks(targetLanguage?: string, platformLanguage?: string) {
        const subtitleTrackChoices = this._syncedData?.subtitles ?? [];
        let tracks = {
        ...
        if (!subtitleTrackChoices.length && emptyChoice) {
            ...
            const normalizedPlatform = normalizeLangCode(platformLanguage);

            // Helper to find a matching track by code or label
            const findTrack = (langCode: string | undefined) => {
                if (!langCode) return undefined;
                // 1. Exact meta match
                let match = subtitleTrackChoices.find((t) => normalizeLangCode(t.language) === langCode);
                if (match) return match;
                // 2. Label match
                match = subtitleTrackChoices.find((t) => normalizeLangCode(t.label) === langCode);
                return match;
            };

            // Helper to detect language from content if simple metadata match failed
            const findTrackDeep = async (langCode: string | undefined): Promise<VideoDataSubtitleTrack | undefined> => {
                if (!langCode) return undefined;

                // Iterate candidates that have a URL and aren't obviously another language
                // We limit to first 3 candidates to avoid performance hit
                const candidates = subtitleTrackChoices
                    .filter((t) => t.url && t.url !== '-' && t.url !== 'lazy')
                    .slice(0, 5);

                for (const track of candidates) {
                    // Skip if already assigned or obvious
                    const obviousLang = normalizeLangCode(track.language) || normalizeLangCode(track.label);
                    if (obviousLang && obviousLang !== langCode) continue;

                    try {
                        // Fetch snippet (first 2KB)
                        let url = typeof track.url === 'string' ? track.url : track.url[0];
                        ...
                        console.warn('Deep detection failed for track', track.label, e);
                    }
                }
                return undefined;
            };

            // --- 1. Target Language (Slot 0) ---
            if (normalizedTarget) {
                let targetTrack = findTrack(normalizedTarget);
                // If simple match fails, try deep detection
                if (!targetTrack && subtitleTrackChoices.length > 0) {
                    targetTrack = await findTrackDeep(normalizedTarget);
                }

                if (targetTrack) {
                    tracks.autoSelectedTracks[0] = targetTrack;
                }
            }

            // --- 2. Platform Language (Slot 1) ---
            // Only if different from target or if target wasn't found (to avoid duplicate unless intent is distinct)
            if (normalizedPlatform && normalizedPlatform !== normalizedTarget) {
                let platformTrack = findTrack(normalizedPlatform);
                // If simple match fails, try deep detection
                // Note: We avoid re-scanning if possible, but safe to try if needed
                if (!platformTrack && subtitleTrackChoices.length > 0) {
                    platformTrack = await findTrackDeep(normalizedPlatform);
                }

                if (platformTrack) {
                    tracks.autoSelectedTracks[1] = platformTrack;
                }
            }

            // --- 3. Fill remaining/fallback from history ---

            // Re-eval matches for "Complete Match" status
            let matches = 0;

            // Check Slot 0 (Target)
            if (tracks.autoSelectedTracks[0].id !== '-') {
                matches++; // We found the target!
            } else if (this.lastLanguagesSynced.length > 0 && this.lastLanguagesSynced[0] !== '-') {
                // Fallback to history only if we failed to find target
                const histLang = this.lastLanguagesSynced[0];
                const histTrack = subtitleTrackChoices.find((t) => t.language === histLang);
                if (histTrack) {
                    tracks.autoSelectedTracks[0] = histTrack;
                    matches++;
                }
            } else {
                matches++;
            ...
            // Check Slot 1 (Platform)
            if (tracks.autoSelectedTracks[1].id !== '-') {
                matches++;
            } else if (this.lastLanguagesSynced.length > 1 && this.lastLanguagesSynced[1] !== '-') {
                const histLang = this.lastLanguagesSynced[1];
                const histTrack = subtitleTrackChoices.find((t) => t.language === histLang);
                if (histTrack) {
                    tracks.autoSelectedTracks[1] = histTrack;
                    matches++;
                }
            } else {
                matches++;
            ...
            // Check Slot 2 (History only)
            if (this.lastLanguagesSynced.length > 2 && this.lastLanguagesSynced[2] !== '-') {
                const histLang = this.lastLanguagesSynced[2];
                const histTrack = subtitleTrackChoices.find((t) => t.language === histLang);
                if (histTrack) {
                    tracks.autoSelectedTracks[2] = histTrack;
                    matches++;
                }
            } else {
                matches++;
            ...
            // Logic for "Complete Match" (Auto-start)
            if (normalizedTarget && tracks.autoSelectedTracks[0].id !== '-') {
                tracks.completeMatch = true;
            } else if (matches >= 3) {
                tracks.completeMatch = true;
            }
