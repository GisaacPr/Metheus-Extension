...
import { SettingsProvider, supportedLanguages as defaultSupportedLanguages } from '@metheus/common/settings';
...
export const fetchLocalization = async (lang: string): Promise<Localization> => {
    // Always prefer bundled strings if available to ensure latest translations are used
    const bundled = await bundledStringsForLang(lang);
    if (bundled) {
        return bundled;
    }

    return (await cachedStringsForLang(lang)) ?? (await bundledStringsForLang('en'))!;
};
