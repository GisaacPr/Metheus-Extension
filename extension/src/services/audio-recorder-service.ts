...
export default class AudioRecorderService {
    ...
    async startWithTimeout(time: number, encodeAsMp3: boolean, requester: Requester): Promise<string> {
        ...
        try {
            ...
            try {
                response = await this._delegate.startWithTimeout(time, encodeAsMp3, requestId, requester);
            } catch (error) {
                throw this._handleStartError(this._delegateThrownErrorToResponse(error), requester);
            }
    ...
    async start(requester: Requester) {
        try {
            ...
            let response: StartRecordingResponse;
            try {
                response = await this._delegate.start(requestId, requester);
            } catch (error) {
                throw this._handleStartError(this._delegateThrownErrorToResponse(error), requester);
            }
    ...
    private _handleStartError(response: StartRecordingResponse, { tabId, src }: Requester): Error {
        ...
        switch (errorCode) {
            case StartRecordingErrorCode.noActiveTabPermission:
                ...
                this._notifyError(
                    'Audio permission is required. Click the extension button in the browser toolbar to enable audio recording for this tab.',
                    { tabId, src }
                );
    ...
    private _delegateThrownErrorToResponse(error: any): StartRecordingResponse {
        ...
        const isPermissionError =
            ...
            normalized.includes('tabcapture') ||
            normalized.includes('getmediastreamid');

        return {
            started: false,
            error: {
                code: isPermissionError ? StartRecordingErrorCode.noActiveTabPermission : StartRecordingErrorCode.other,
                message: message || 'Failed to start audio recording',
            },
        };
    }
}
