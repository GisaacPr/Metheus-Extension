import { OffscreenDomCache } from '@metheus/common';
    ...
    *displayingElements() {
    ...
    setHtml(htmls: KeyedHtml[]) {
        if (document.fullscreenElement) {
            ...
            this._clearChildren(this.nonFullscreenContainerElement);
        } else {
            this._displayNonFullscreenContentElementsWithHtml(htmls);
            this._clearChildren(this.fullscreenContainerElement);
        }
    }

    private _clearChildren(containerElement: HTMLElement | undefined) {
        if (!containerElement) {
            return;
        }
    ...
    private _applyContainerStyles(container: HTMLElement) {
        ...
        const isFullscreenApi = document.fullscreenElement !== null;
        const isRightAligned = this.horizontalAlign === 'right';

        if (isRightAligned) {
            container.style.setProperty('left', 'auto', 'important');
            const rightOffset = Math.max(0, viewportWidth - rect.right) + 24; // 24px padding from right edge
            container.style.setProperty('right', Math.round(rightOffset) + 'px', 'important');
            container.style.setProperty('transform', 'none', 'important');
        } else {
            container.style.left = rect.left + rect.width / 2 + 'px';
            container.style.right = '';
            container.style.transform = 'translate(-50%)'; // The original class has no transform in styles here, but it's likely in CSS. Wait, the CSS had it!
            // In AsbPlayer CSS: transform: translate(-50%) !important;
            // So we don't strictly need to set it here for 'center' unless we want to be explicit, but we DO need to unset it for 'right'.
            // Actually, the CSS has !important on transform, so unsetting via style='' might not work if it's in the class.
            // But we are using setProperty(..., 'important').

            // Re-reading CSS: .asbplayer-mobile-video-overlay-container-top { transform: translate(-50%) !important; }
            // So for center, we rely on CSS.
        ...
        if (this.contentWidthPercentage === -1) {
            ...
            container.style.width = Math.min(viewportWidth, (rect.width * this.contentWidthPercentage) / 100) + 'px';
        }

        const baseY = isFullscreenApi ? viewportOffsetTop + rect.top : rect.top + window.scrollY;
        const clampedY = Math.max(baseY, 0);

        if (this.offsetAnchor === OffsetAnchor.bottom) {
            const baseBottom = isFullscreenApi ? viewportOffsetTop + rect.top + rect.height : clampedY + rect.height;
            const maxVisibleBottom = isFullscreenApi
                ? viewportOffsetTop + viewportHeight
                : window.innerHeight + window.scrollY;
            const clampedBottom = Math.min(baseBottom, maxVisibleBottom);
            container.style.top = clampedBottom - this.contentPositionOffset + 'px';
