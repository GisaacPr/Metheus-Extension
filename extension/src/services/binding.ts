...
import { extractText, seekWithNudge, sourceString, surroundingSubtitlesAroundInterval } from '@metheus/common/util';
...
import { bufferToBase64 } from '@metheus/common/base64';
import { pgsParserWorkerFactory } from './pgs-parser-worker-factory';
import { initializeMetheus, cleanupMetheus } from './metheus-integration';
...
export default class Binding {
    ...
    constructor(video: HTMLMediaElement, hasPageScript: boolean, settings?: SettingsProvider, frameId?: string) {
        this.video = video;
        this.hasPageScript = hasPageScript;
        // E-M1 FIX: Use shared SettingsProvider to avoid inconsistent state across bindings
        this.settings = settings ?? new SettingsProvider(new ExtensionSettingsStorage());
    ...
    _bind() {
        ...
        initializeMetheus(this.settings, this.subtitleController)
            .then((controller) => {
                if (controller && controller.enabled) {
                    console.log('[Binding] Metheus initialized with native subtitle support');
                }
            })
            .catch((err) => {
                console.error('[Binding] Failed to initialize Metheus:', err);
            });
    ...
    _subscribe() {
        ...
        // LN UX: Pause-on-hover is disabled globally.
        // Video pause/resume is now controlled by dictionary word-click popup lifecycle.
        this.subtitleController.onMouseOver = undefined;
            ...
            if (request.sender === 'asbplayer-extension-to-video' && request.src === this.video.src) {
                switch (request.message.command) {
                    ...
                    case 'load-subtitles':
                        ...
                        break;
                    case 'show-mobile-overlay':
                        this.mobileVideoOverlayController.unhideFromPopupAction();
                        this.mobileVideoOverlayController.updateModel();
                        break;
    ...
    async _refreshSettings() {
        ...
        if (currentSettings.streamingEnableOverlay) {
            this.mobileVideoOverlayController.offsetAnchor = OffsetAnchor.top;
    ...
    async rerecord(start: number, end: number) {
    ...
    private _updateSubtitles(subtitles: IndexedSubtitleModel[], subtitleFileNames: string[]) {
        ...
        // Always show the mobile overlay when synced, so it appears immediately
